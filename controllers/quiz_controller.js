...
exports.index = function(req, res){
	...
	if(req.query.search) {
		models.Quiz.findAll({where:["pregunta like ?", '%' + req.query.search + '%'], order:'pregunta ASC'}).then(function(quizes) {
			res.render('quizes/results', {quizes: quizes, errors: []});
	...
	.then(
		function(err){
			if (err){
				...
				quiz // Guarda en la BBDD los campos pregunta y respuesta de quiz
	...
	.then(
		function(err){
			if (err){
				...
				req.quiz // Guarda en la BBDD los campos pregunta y respuesta de quiz
				.save({fields: ["pregunta", "respuesta", "tema"]})
				.then(function(){res.redirect('/quizes');}); 
			}	// Redireccion HTTP (URL relativo) lista de preguntas 	
		}
